...
def class_acc(y_pred, y_test, num_classes):    
    output = torch.argmax(y_pred, dim=1)
    return accuracy(output, y_test), f1(output, y_test, average='macro', num_classes=num_classes)

def train(model, optimizer, train_loader, val_loader, device, num_classes=3, epochs=20, save=True, saved_folder="saved", train_writer=None, val_writer=None):
    '''
    ...
    for e in range(1, epochs+1):
        ...
        for (X_batch, y_batch) in tqdm(train_loader):
            ...
            acc = class_acc(y_pred, y_batch, num_classes=num_classes)
        ...
        with torch.no_grad(): 
            ...
            for x_val, y_val in val_loader:  
                ...
                acc = class_acc(yhat, y_val.squeeze(), num_classes=num_classes)
                val_acc += acc[0]
                val_f1_score += acc[1]
        ...
        if save:
            if best_val_f1 < val_f1_score or best_val_loss >= val_loss:
                ...
                torch.save({
                    ...
                    'loss': epoch_loss,
                    }, f"{saved_folder}/resnet18_age_.5_hflip_{e}_{epoch_loss/len(train_loader):.2f}_{epoch_acc/len(train_loader):.2f}.pt")
