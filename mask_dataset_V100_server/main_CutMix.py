...
from module.F1_score import F1_Loss # ToDo : loss_combination과 중복 계산 중 중복 제거할 것
from module.CutMix import rand_bbox
...
hp = {
    ...
    "IMAGE_SIZE_W"  : 128,
    "SUB_MEAN"      : False,
    "EXP_NUM"       : "CutMix_pretrained_SGD%s"%datetime.now().strftime("%Y-%m-%d_%H_%M_%S"),
    "DEBUG"         : False,
...
resnet20.fc = torch.nn.Linear(
    in_features= 512, out_features=18, bias=True
)
    ...
    for X, y in iter(train_dataloader):
        ...
        #= train epoch recording ==================
            ...
            if BETA > 0 and np.random.random()>0.5: # cutmix가 실행될 경우
                ...
                target_a = y.to(DEVICE) # 원본 이미지 label
                target_b = y.to(DEVICE)[rand_index] # 패치 이미지 label
                ...
                predict = resnet20(X.to(DEVICE))
                #loss_val = loss_combination(predict, y.to(DEVICE))
                loss_val = loss_combination(predict, target_a) * lam
                loss_val += loss_combination(predict, target_b) * (1. - lam) # 패치 이미지와 원본 이미지의 비율에 맞게 
