...
class F1_Loss(nn.Module):
    ...
    '''
    def __init__(self,num_classes, epsilon=1e-7, test = False):
        super().__init__()
    ...
    def forward(self, y_pred, y_true,):
        ...
        if self.test:
            #  only for TDD, DO NOT activate these lines on training phase 
            y_pred = torch.max(y_pred,1)
            y_pred = F.one_hot(y_pred[1], self.num_classes).to(torch.float32)
